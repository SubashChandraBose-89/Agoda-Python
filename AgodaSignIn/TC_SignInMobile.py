...
class SigninMobile(unittest.TestCase):
    def setUp(self):
    ...
    def test_blankEmailLogin(self):
        ...
        self.expect_result = "Please enter a valid mobile number."
        self.assertTrue(self.actual_result == self.expect_result), "failed"
        print("Blank Mobile Login Alert is Successful")

    def test_validMobileLogin(self):
        self.wait = WebDriverWait(self.driver, 10)
        self.popup_1 = self.wait.until(EC.element_to_be_clickable((By.XPATH, "//button[@aria-label='Close Message']")))
        self.popup_1.click()
        self.driver.find_element(By.XPATH, '//span[@class="Box-sc-kv6pi1-0 bTkTcE"]').click()
